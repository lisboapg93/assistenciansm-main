-- Centralize privileged session and inventory writes in the database.

-- Editors are operational users, not role administrators. Role changes must be
-- performed by a trusted administrative backend using the service role.
...
-- Assistants may register a session only through the constrained RPC below.
...
-- Restore editor-only direct inventory and member maintenance. Assistants do
-- not need these broad table permissions to register a session.
DROP POLICY IF EXISTS "Editors and assistants can insert vegetal" ON public.vegetal;
DROP POLICY IF EXISTS "Editors and assistants can update vegetal" ON public.vegetal;
DROP POLICY IF EXISTS "Editors and assistants can insert stock_movement" ON public.stock_movement;
DROP POLICY IF EXISTS "Editors and assistants can insert members" ON public.members;

CREATE POLICY "Editors can insert vegetal"
  ON public.vegetal FOR INSERT TO authenticated
  WITH CHECK (public.has_role(auth.uid(), 'editor'::public.app_role));

CREATE POLICY "Editors can update vegetal"
  ON public.vegetal FOR UPDATE TO authenticated
...
BEGIN
  ...
  -- Lock inventory rows in a stable order before mutating balances.
  FOR v_source IN
  ...
  LOOP
    IF v_source.amount_available IS NULL OR v_source.amount_available <= 0
      OR v_source.quantity < CASE WHEN v_is_united THEN v_source.amount_available ELSE v_total_consumed END THEN
  ...
  INSERT INTO public.session (
    ...
    chamadas, historias, has_photo, has_audio
  ) VALUES (
    ...
    NULLIF(p_session->>'chamadas', ''),
    NULLIF(p_session->>'historias', ''),
...
-- Match the most common filtering and ordering patterns used by the history
-- and inventory detail screens.
CREATE INDEX IF NOT EXISTS idx_session_type_date ON public.session(type, date DESC);
CREATE INDEX IF NOT EXISTS idx_stock_movement_vegetal_date ON public.stock_movement(vegetal_id, date DESC);
